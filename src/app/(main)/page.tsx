...
const getCachedMVPData = unstable_cache(
  async () => {
    try {
      ...
      const topKill = [...lastFinishedC4.statistics].sort(
        (a, b) => (b.killGain || 0) - (a.killGain || 0)
      )[0];

      const topPower = [...lastFinishedC4.statistics].sort(
        (a, b) => (b.powerGain || 0) - (a.powerGain || 0)
      )[0];

      const topKD = [...lastFinishedC4.statistics].sort(
        (a, b) => (b.kdGain || 0) - (a.kdGain || 0)
      )[0];

      const topDie = [...lastFinishedC4.statistics].sort(
        (a, b) => (b.dieGain || 0) - (a.dieGain || 0)
      )[0];

      const topResource = [...lastFinishedC4.statistics].sort((a, b) => {
      ...
      const mvps = [
...

        {
          ...
          value: formatLargeNumber(
            topResource?.player?.resourceCollection || BigInt(0)
          ),
        },
      ];
      ...
      const serializableMvps = mvps.map((mvp) => ({
        ...mvp,
        value:
          typeof mvp.value === "bigint"
            ? formatLargeNumber(mvp.value)
            : mvp.value,
        player: mvp.player
          ? {
              ...mvp.player,
              startResourceCollection: mvp.player.startResourceCollection
                ? formatLargeNumber(mvp.player.startResourceCollection)
                : null,
              resourceCollectionGain: mvp.player.resourceCollectionGain
                ? formatLargeNumber(mvp.player.resourceCollectionGain)
                : null,
              player: mvp.player.player
                ? {
                    ...mvp.player.player,
                    resourceCollection: mvp.player.player.resourceCollection
                      ? formatLargeNumber(mvp.player.player.resourceCollection)
                      : null,
                  }
                : null,
            }
      ...
      return {
        mvps: serializableMvps.filter(
          (item) => item.player || item.category === "placeholder"
        ),
  ...
  return (
    <>
      <div className="container mt-4 md:mt-12 mx-auto flex flex-wrap p-2 md:p-4 rounded-xl">
        ...
        <div className="w-full mt-4 grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4 justify-center">
          {/* Блок с промокодами */}
          <Card className="md:col-span-1 rounded-2xl flex flex-col">
            ...
            <div className="flex justify-center mb-4">
              ...
              >
                <Badge className="bg-slate-200 px-4 py-2" variant="outline">
                  Подробнее
                </Badge>
              </Link>
            </div>
          ...
          <div className="md:col-span-1">
            <C4Card
              status={currentC4.status}
              players={currentC4.totalPlayers}
              map={currentC4.map}
            />
          </div>
        </div>
