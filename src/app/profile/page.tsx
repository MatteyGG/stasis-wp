...
export default async function ProfilePage() {
  ...
  function calculateForcePercentages(
    ground: number,
    air: number,
    navy: number
  ) {
  ...
  const c4History: C4Data[] = c4HistoryData.map((stat) => {
    const startDate = stat.c4.startedAt
      ? new Date(stat.c4.startedAt).toLocaleDateString("ru-RU")
      : "Неизвестно";
    const endDate = stat.c4.endedAt
      ? new Date(stat.c4.endedAt).toLocaleDateString("ru-RU")
      : "Неизвестно";
      ...
      {playerStats && (
        <Card className="mt-4 rounded-2xl">
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
              <div className="text-center">
                ...
                <div className="text-2xl font-bold">
                  {formatLargeNumber(playerStats.power)}
                </div>
              </div>
              <div className="text-center">
              ...
              <div className="text-center">
                ...
                <div className="text-2xl font-bold">
                  {formatLargeNumber(playerStats.kill)}
                </div>
              </div>
              <div className="text-center">
                <Label className="text-sm text-muted-foreground">Смерти</Label>
                <div className="text-2xl font-bold">
                  {formatLargeNumber(playerStats.die)}
                </div>
              </div>
              <div className="text-center">
                <Label className="text-sm text-muted-foreground">
                ...
                <div className="text-2xl font-bold">
                  {formatLargeNumber(playerStats.resourceCollection)}
                </div>
              </div>
              <div className="text-center">
                <Label className="text-sm text-muted-foreground">
            ...
            <UpdatePlayerButton lastUpdated={fullUser.player?.lastUpdated} />
          </CardContent>
        </Card>
      )}
      {/* таблица C4 */}
